// modules/conversion/checkout/CheckoutFlow.tsx
// Owns: In-app payment flow surface
// Renders the ported PayMe checkout card against live cart items.

import { useCallback, useMemo, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useResolvers } from '../../../resolver-boundary/ResolverProvider';
import { PayMeCheckoutCard } from './PayMeCheckoutCard';
  ...
  const handlePay = useCallback(
    async (_totalDueToday: number, couponCode?: string) => {
      ...
      try {
        ...
        // Build setup/monthly line items from pricing table
        ...
        // Apply discount codes to SETUP subtotal (matches PayMe UI)
        ...
        // Apply the discount to setup items (reduce first items until exhausted)
        ...
        // Remove any fully discounted setup lines (Stripe disallows 0 amounts)
        const finalSetupItems = setupItems.filter((it) => it.unit_amount >= 1);

        const lineItems = [...finalSetupItems, ...monthlyItems];
